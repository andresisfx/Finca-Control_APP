...
def create_produccion(produccion_in: ProduccionVendedorDiariaCreate, db: Session = Depends(get_db)):
    """
    Registra los litros de leche entregados por un vendedor en una fecha específica.
    """
...
def read_producciones_por_vendedor(vendedor_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """
    Obtiene el historial de producción (litros entregados) por un vendedor, ordenado por fecha.
    """
...
def read_produccion(produccion_id: int, db: Session = Depends(get_db)):
    """
...
def update_produccion(produccion_id: int, produccion_in: ProduccionVendedorDiariaUpdate, db: Session = Depends(get_db)):
    """
    Actualiza un registro de producción (ej. corregir litros ingresados).
    """
