...
def create_quincena(quincena_in: QuincenaCreate, db: Session = Depends(get_db)):
    """Crea una nueva quincena (periodo de facturación) para una finca."""
...
def read_quincenas_por_finca(finca_id: UUID4, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Lista las quincenas de una finca, ordenadas desde la más reciente."""
...
def read_quincena(quincena_id: int, db: Session = Depends(get_db)):
    """Obtiene los detalles de una quincena específica."""
    return quincena_service.get_quincena(db=db, quincena_id=quincena_id)

@router.put("/{quincena_id}", response_model=QuincenaOut)
def update_quincena(quincena_id: int, quincena_in: QuincenaUpdate, db: Session = Depends(get_db)):
    """Actualiza las fechas de inicio o fin de una quincena."""
...
def delete_quincena(quincena_id: int, db: Session = Depends(get_db)):
    """Elimina una quincena y todos los registros asociados en cascada."""
