...
def create_vendedor(vendedor_in: VendedorCreate, db: Session = Depends(get_db)):
    """
...
def read_vendedores_por_finca(finca_id: UUID4, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """
...
def update_vendedor(vendedor_id: int, vendedor_in: VendedorUpdate, db: Session = Depends(get_db)):
    """
    Actualiza la información (nombre, teléfono) de un vendedor.
    """
...
def delete_vendedor(vendedor_id: int, db: Session = Depends(get_db)):
    """
    Elimina un vendedor (Cascada eliminará todo su historial de producción).
    """
