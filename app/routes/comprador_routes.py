...
def create_comprador(comprador_in: CompradorCreate, db: Session = Depends(get_db)):
...
def read_compradores_por_finca(finca_id: UUID4, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
...
def update_comprador(comprador_id: int, comprador_in: CompradorUpdate, db: Session = Depends(get_db)):
