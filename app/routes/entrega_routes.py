...
def create_entrega(entrega_in: EntregaCreate, db: Session = Depends(get_db)):
    """Registra los litros de leche entregados a un comprador en una fecha específica de una quincena."""
...
def read_entregas_por_quincena(quincena_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """Obtiene el historial de despachos/entregas realizadas en una quincena."""
...
def read_entrega(entrega_id: int, db: Session = Depends(get_db)):
    """Obtiene los detalles de un despacho de leche específico."""
    return entrega_service.get_entrega(db=db, entrega_id=entrega_id)

@router.put("/{entrega_id}", response_model=EntregaOut)
def update_entrega(entrega_id: int, entrega_in: EntregaUpdate, db: Session = Depends(get_db)):
