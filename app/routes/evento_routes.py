...
def create_evento(evento_in: EventoCreate, db: Session = Depends(get_db)):
    """
    Registra un nuevo evento (vacuna, parto, enfermedad, etc.) para un animal.
    """
    return evento_service.create_evento(db=db, evento_in=evento_in)

@router.get("/animal/{animal_id}", response_model=list[EventoOut])
def read_eventos_por_animal(animal_id: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    """
    Obtiene el historial de eventos de un animal, ordenados por fecha descendente.
    """
...
def update_evento(evento_id: int, evento_in: EventoUpdate, db: Session = Depends(get_db)):
    """
